...
function saveMeal(date, mealType, value) {
    let mealData = JSON.parse(localStorage.getItem('schoolMeals')) || {};
    if (!mealData[date]) mealData[date] = {};
    mealData[date][mealType] = value;
    localStorage.setItem('schoolMeals', JSON.stringify(mealData));
}
...
document.querySelectorAll('.school-meal').forEach(section => {
    section.addEventListener('dblclick', () => {
        ...
        paragraph.addEventListener('blur', function handler() {
            ...
            document.querySelectorAll('#weekly-timetable-content .school-meals').forEach(td => {
...
document.querySelector('.modal-enter-btn').addEventListener('click', () => {
    if (activeTd) {
        ...
        document.querySelectorAll('.school-meal').forEach((section, idx) => {
            const startDateInput = document.getElementById('start-date');
...
document.getElementById('start-date').addEventListener('change', () => {
    ...
    document.querySelectorAll('#weekly-timetable-content .school-meals').forEach((td, tdIdx) => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const dailyRadio = document.getElementById('schoolDailyMealPlan');
    const weeklyRadio = document.getElementById('schoolWeeklyMealPlan');
...
function printVisibleCard() {
    // Get both cards
    const dailyCard = document.querySelector('.school-lunch-plan-card-container');
    const weeklyCard = document.querySelector('.school-lunch-weekly-plan-card-container');
    ...
    const visibleCard = document.querySelector('.school-lunch-plan-card-container.flex, .school-lunch-weekly-plan-card-container.flex');
    if (!visibleCard) {
